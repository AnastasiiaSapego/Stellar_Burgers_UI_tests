...
public class RegistrationForm extends BasePage {
    ...
    private final By nameInput =
            By.xpath("//form//fieldset[.//label[normalize-space()='Имя']]//input[contains(@class,'input__textfield')]");
    ...
    private final By registerButton =
            By.xpath("//form//button[normalize-space()='Зарегистрироваться']");
    private final By signInLink =
            By.xpath("//p/a[normalize-space()='Войти']");

    private final By wrongPasswordError =
    ...
    public void waitOpened() { visible(nameInput); }

    @Step("Заполнить имя: {name}")
    public void setNameField(String name) { type(nameInput, name); }

    @Step("Заполнить email: {email}")
    public void setEmailField(String email) { type(emailInput, email); }

    @Step("Заполнить пароль (длина={password.length})")
    public void setPasswordField(String password) { type(passwordInput, password); }

    @Step("Нажать кнопку 'Зарегистрироваться'")
    public void clickRegistrationButton() { clickable(registerButton).click(); }

    @Step("Нажать ссылку 'Войти' в форме регистрации")
    public void clickSignInButtonInRegistrationForm() { clickable(signInLink).click(); }
